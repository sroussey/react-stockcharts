...
function helper(props, moreProps) {
  ...
  if (dx !== 0 && dy !== 0) {
    ...
    const halfY = getLineCoordinates(startXY, x2, y1 + dy / 2, '2/1');
    const oneThirdY = getLineCoordinates(startXY, x2, y1 + dy / 3, '3/1');
    const oneFourthY = getLineCoordinates(startXY, x2, y1 + dy / 4, '4/1');
    const oneEighthY = getLineCoordinates(startXY, x2, y1 + dy / 8, '8/1');
    const halfX = getLineCoordinates(startXY, x1 + dx / 2, y2, '1/2');
    const oneThirdX = getLineCoordinates(startXY, x1 + dx / 3, y2, '1/3');
    const oneFourthX = getLineCoordinates(startXY, x1 + dx / 4, y2, '1/4');
    const oneEighthX = getLineCoordinates(startXY, x1 + dx / 8, y2, '1/8');
    const lines = [
      oneEighthX,
      oneFourthX,
      oneThirdX,
