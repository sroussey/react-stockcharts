...
class ZoomAndPanPage extends React.Component {
  ...
  render() {
    const { mouseMoveEvent, panEvent, zoomEvent, clamp } = this.state;
      ...
      <ContentSection title={ZoomAndPanPage.title}>
        ...
        <Row>
          <div style={{ textAlign: 'center' }}>
            <button
              type='button'
              onClick={this.toggleState.bind(this, 'mouseMoveEvent')}>
        ...
        <Row>
          <Section colSpan={2}>
            <aside
              dangerouslySetInnerHTML={{ __html: require('md/ZOOM-AND-PAN') }}
            />
          </Section>
        </Row>
      </ContentSection>
