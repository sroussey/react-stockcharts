...
import { MainContainer } from 'lib/MainContainer';
import { MenuGroup } from 'lib/MenuGroup';
...
function loadPage() {
  ...
  const promiseBubbleData = fetch('data/bubble.json').then((response) =>
    response.json()
  );
  const promiseBarData = fetch('data/barData.json').then((response) =>
    response.json()
  );
  const promisegroupedBarData = fetch('data/groupedBarData.json').then(
...
function renderPage(
  ...
  const selectedPage = pages.filter(
    (page) => compressString(page.title) === compressString(selected)
  );
